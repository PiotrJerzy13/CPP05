...
/*   Updated: 2025/02/08 17:54:21 by pwojnaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void testInvalidBureaucrats()
{
    ...
    try
    {
        Bureaucrat tooHigh("HighRank", 0);  // Should throw GradeTooHighException
    }
    catch (const std::exception &e)
    {
    ...
    try
    {
        Bureaucrat tooLow("LowRank", 151); // Should throw GradeTooLowException
    }
    catch (const std::exception &e)
    {
        std::cerr << "Caught exception: " << e.what() << std::endl;
...
void testIncrementBeyondLimit()
{
    ...
    try
    {
        ...
        top.incrementGrade(1); // Should throw GradeTooHighException
    }
    catch (const std::exception &e)
    {
        std::cerr << "Caught exception: " << e.what() << std::endl;
...
void testDecrementBeyondLimit()
{
    ...
    try
    {
        ...
        bottom.decrementGrade(1); // Should throw GradeTooLowException
    }
    catch (const std::exception &e)
    {
        std::cerr << "Caught exception: " << e.what() << std::endl;
